...
const handleStorage = (spawn: StructureSpawn) => {
  const controllerLevel = spawn.room.controller?.level ?? 0;
  if(controllerLevel < 4) return
...
const ensureIndex = (index: number, spawn: StructureSpawn) => {
...
const setRoadComplete = (index: number, spawn: StructureSpawn) => {
  ensureIndex(index, spawn);

  spawn.memory.roadLevelsPlaced[index] = true;
}

const isRoadLevelComplete  = (index: number, spawn: StructureSpawn) => {
  ensureIndex(index, spawn);

  return spawn.memory.roadLevelsPlaced[index]
}

const resetRoadLevelStatuses = (spawn: StructureSpawn) => {
  for(const val in spawn.memory.roadLevelsPlaced){
    spawn.memory.roadLevelsPlaced[val] = false;
  }
}

const removeAllRoads = (spawn: StructureSpawn) => {
...
const handleRoads = (spawn: StructureSpawn): void => {
  ...
  const handleLevel2 = () => {
    ...
    if(isRoadLevelComplete(2, spawn)) return;
  ...
  const handleLevel3 = () => {
    ...
    if(isRoadLevelComplete(3, spawn)) return;
